...
const mapped = [1, 2, 3].map((item) => { return item * 2; });
...
async function promiseReturnWrap(): Promise<number> {
	return Promise.resolve(42).then(async (value) => {
		return value;
	});
}
...
const boundFunc = function() { return 42; };
...
void [
	...
	undefinedInit, foo, value, logicalOr, testLonelyIf,
	found, deMorgan, unsortedObject, UnsortedEnum,
	items, duplicateStrings, collapsibleIf, immediateReturn,
	redundantBoolean, invertedCheck, promiseReturnWrap, tooManyParameters,
	MissingAccessibility, power, oneA, oneB, hex, computedKey,
	boundFunc, returnUndefined, testSpread, uselessReturn,
